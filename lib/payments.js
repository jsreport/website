...
module.exports.updatePaymentMethod = async (customerId, productId, pm, db) => {
  logger.info(`updating patyment method for customer: ${customerId}, productId: ${productId}`)
  const product = await findProduct(customerId, productId, db)
  ...
  const sres = await gateway.subscription.update(product.braintree.subscription.id, {
    paymentMethodToken: pmr.paymentMethod.token
  })
...
module.exports.cancelSubscription = async (customerId, productId, db) => {
  ...
  const cancelResult = await gateway.subscription.cancel(product.braintree.subscription.id)
...
async function findProduct (customerId, productId, db) {
  const customer = await db.collection('customers').findOne({ uuid: customerId })
  if (!customer) {
    throw new Error('Customer not found')
  }
